...
The graph layer lives in `~/.nuggets/graph/graph.json` and is shared across all nuggets in the same `saveDir`. All types below are exported from the `nuggets` package.
